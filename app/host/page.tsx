        ...
        <div>
          ...
          <div style={{display:'flex', gap:8, flexWrap:'wrap'}}>
            {[1,2,3,4,5,6,7,8].map(n => (
              <div key={n} style={{border:'1px solid #1f2937', borderRadius:8, padding:8, minWidth:120}}>
                <div><strong>Round {n}</strong></div>
          ...
          <div style={{display:'grid', gap:8, maxWidth:520}}>
            <input type="number" min={1} max={8} value={qsRound} onChange={e=>setQsRound(parseInt(e.target.value,10))} />
            <input type="number" min={1} max={20} value={qNum} onChange={e=>setQNum(parseInt(e.target.value,10))} />
            <input placeholder="Question prompt" value={qText} onChange={e=>setQText(e.target.value)} />
